...
```
Tras observar la grafica a simple vista podemos decir que es estacionaria, ademas que en el año 2020 la importacion de gasolina Regular tuvo una 'crisis' esto puede considerarse por la pandemia del COVID-19. Y que antes de la pandemia tuvo un pico considerable. 
...
```
como se observa tiene estacionalidad por lo que no es estacionaria en varianza y tiene tendencia por lo que no es estacionaria en media
```{r}
...
```
Aplicamos transformacion logaritmica debido que nuestra serie de tiempo tiene un sesgo positivo. 
```{r}
...
```
Tal como se habia mencionado anteriormente, esta serie de tiempo es estacionaria, ademas se obtuvo un valor de P de 0.01 indicando que tiene una correlacion positiva y respaldando el hecho de aplicar una funcion logaritmica a los datos. 
```{r}
# Hacer el modelo
...
### Serie de tiempo de gasolina Super

```{r}

super_ts <- ts( super$`Gasolina superior`, start = c(2001,1),frequency = 12)

#Donde comienza la serie de tiempo
start(super_ts)
#Donde termina la serie de tiempo
end(super_ts)
...
```
Tras observar la grafica a simple vista podemos decir que es estacionaria, ademas que en el año 2020 la importacion de gasolina Superior tuvo una 'crisis' esto puede considerarse por la pandemia del COVID-19.Ademas, tambien se observa que en años siguientes la importacion tuvo un pico. 


```{r}

plot(aggregate(super_ts,FUN=mean))
dec.Regular<-decompose(super_ts)
plot(dec.Regular)
plot(dec.Regular$seasonal)
...
### Serie de tiempo de gasolina Diesel
...
dec.Regular<-decompose(diesel_ts)
plot(dec.Regular)
plot(dec.Regular$seasonal)

#Aplicaremos una transformación logarítmica
logDieselr <- log(diesel_ts)
plot(decompose(logDieselr))
...
#Para saber si hay raíces unitarias
adfTest(logDieselr)
adfTest(diff(logDieselr))

#Gráfico de autocorrelación
acf(logDieselr)

# funciones de autocorrelación y autocorrelación parcial
acf(diff(logDieselr),12)
pacf(diff(logDieselr))
